...
\begin{enumerate}[label=(\alph*)]
	...
	\item 
		...
		Using \textit{Construction 4.24} with the field $\mathbb{F}=\ZZ_5$, we can construct an affine plane of order $5$.  From the proof of \textit{Theorem 4.38} an affine plane of order $n=5$ with is a balanced block design with parameters:
		\[
	...
	\item 
		...
		\begin{center}
			\begin{tikzpicture}
				...
				\node[label={[above,right,red]$q_1$}] (q1) at (0,0) {};
				\node[label={[above,right,red]$q_2$}] (q2) at (1,0) {};
				\node[label={[above,right]$p_1$}] (p1) at (0,-1) {};
				\node[label={[above,right]$p_2$}] (p2) at (1,-1) {};
				\node[label={[above,right]$p_3$}] (p3) at (0.5,-0.5) {};
				\fill[red] (q1) circle (2pt);
				\fill[red] (q2) circle (2pt);
				\fill (p1) circle (2pt);
				\fill (p2) circle (2pt);
				\fill (p3) circle (2pt);
			\end{tikzpicture}
